...
import type { Product, Movement, MovementType, Category } from "@/lib/data";

interface ShopClientProps {
  products: Product[];
  movements: Movement[];
  categories: { id: Category; name: string; description: string }[];
}

function ShopContent({ products, movements, categories }: ShopClientProps) {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return result;
  }, [products, selectedMovements, selectedCategories, selectedGenders, showNew, showBestSellers, sortBy]);
        ...
        <div className="space-y-2">
          {movements.map((movement) => (
            <div key={movement.id} className="flex items-center space-x-2">
              <Checkbox id={`movement-${movement.id}`} checked={selectedMovements.includes(movement.id)} onCheckedChange={() => toggleMovement(movement.id)} />
              <Label htmlFor={`movement-${movement.id}`} className="text-sm font-normal cursor-pointer">{movement.name}</Label>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="space-y-2">
          {categories.map((category) => (
            <div key={category.id} className="flex items-center space-x-2">
              ...
              <Label htmlFor={`category-${category.id}`} className="text-sm font-normal cursor-pointer">{category.name}</Label>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="space-y-2">
          {["women", "men", "unisex"].map((gender) => (
            <div key={gender} className="flex items-center space-x-2">
              <Checkbox id={`gender-${gender}`} checked={selectedGenders.includes(gender)} onCheckedChange={() => toggleGender(gender)} />
              <Label htmlFor={`gender-${gender}`} className="text-sm font-normal cursor-pointer capitalize">{gender}</Label>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between mb-6">
              ...
              <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="text-sm border rounded-sm px-3 py-2 bg-background">
